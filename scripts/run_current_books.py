...
def build(rows, name):
    ...
    return compute_all(Portfolio(holdings=hs), benchmark_choice="^NSEI", risk_profile_key="moderate", risk_free_rate=0.065)
...
def dump(report, name, prices):
    ...
    if ins:
        ...
        for attr in ["overall_risk_score","probability_rating","impact_rating","composite_rating","risk_tier","diversification_score","concentration_risk","liquidity_risk","tail_risk"]:
            v = getattr(ins, attr, None)
            if v is not None: print(f"  {attr:24s}: {v}")
    if rec:
        print(f"\n-- RECOMMENDATIONS --")
        for a in getattr(rec,"actions",[]) or []:
            print(f"  [{a.severity if hasattr(a,'severity') else ''}] {a.ticker if hasattr(a,'ticker') else ''}: {a.rationale if hasattr(a,'rationale') else a}")
        top = getattr(rec,"top_actions",None)
        if top: 
            for a in top: print("  TOP:", a)
