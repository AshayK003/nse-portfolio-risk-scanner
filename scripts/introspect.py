...
def build(rows):
    hs = [
        Holding(ticker=t, name=t, quantity=q, avg_price=a, sector=s, current_price=None)
        for t, q, a, s in rows
    ]
    # force_refresh=True -> bypass 24h disk cache, fetch live
    return compute_all(
        Portfolio(holdings=hs),
        benchmark_choice="^NSEI",
        risk_profile_key="moderate",
...
for nm, rows in [("ASHAY", ASHAY), ("RISHU", RISHU)]:
    ...
    if not ra.empty:
        ...
        for _, row in ra.head(8).iterrows():
            print(
                f"  {row['Ticker']:14s} wt={row['Weight (%)']:5.1f}%  risk%={row['Risk Contrib (%)']:5.1f}%  beta={row['Beta']}  vol={row['Ann. Vol (%)']}%"
            )
    # ---- EARLY WARNINGS ----
    print("\n[EARLY WARNINGS]")
    w = rep.warnings
    if w and getattr(w, "signals", None):
        print(
            f"  overall_level: {getattr(w, 'overall_warning_level', '?')}  | counts: {getattr(w, 'signal_count_by_severity', {})}"
        )
        for s in w.signals:
