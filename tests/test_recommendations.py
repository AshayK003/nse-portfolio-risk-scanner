    ...
    return PortfolioSnapshot(
        holdings=[
            ...
            },
            {
                ...
                "quantity": 70,
                "avg_price": 101.21,
                "current_price": 108.90,
            },
            {
                "ticker": "MONIFTY500.NS",
                "name": "Nifty 500 ETF",
                "quantity": 280,
                "avg_price": 23.61,
                "current_price": 24.13,
...
def _make_market_data(
    regime=RegimeContext.BULL, vix=13.5, adx=25.0, breadth=1.2, fii_dii=FiiDiiBias.NEUTRAL, expiry=False
):
    return MarketData(
        vix=vix,
        adx=adx,
...
def test_sector_concentration_flagged():
    ...
    broad_market_cards = [
        c for c in cards if "Broad Market" in c.reason or "concentration" in c.reason.lower()
    ]
...
def test_vix_spike_blocks_shorts():
    ...
    cards, report = run_recommendation_engine(
        _make_snapshot(), _make_market_data(vix=30), _make_user_profile()
    )
...
def test_cash_floor_triggers_sells():
    ...
    cards, report = run_recommendation_engine(
        _make_snapshot(), _make_market_data(), _make_user_profile(cash_floor=50000)
    )
...
def test_regime_bull_no_vix_spike():
    ...
    cards, report = run_recommendation_engine(
        _make_snapshot(cash_available=60000), _make_market_data(vix=12), _make_user_profile(cash_floor=50000)
    )
...
def test_expiry_week_reduces_positions():
    ...
    cards, report = run_recommendation_engine(
        _make_snapshot(cash_available=60000),
        _make_market_data(expiry=True),
        _make_user_profile(cash_floor=50000),
    )
...
def test_fii_dii_confluence():
    ...
    cards, report = run_recommendation_engine(
        _make_snapshot_positive_bias(),
        _make_market_data(fii_dii=FiiDiiBias.BEARISH),
        _make_user_profile(cash_floor=50000),
    )
...
def test_recommendation_cards_have_required_fields():
    ...
    cards, report = run_recommendation_engine(
        _make_snapshot(cash_available=60000), _make_market_data(), _make_user_profile(cash_floor=50000)
    )
