...
def estimate_garch_var(returns: np.ndarray, confidence: float = 0.95) -> GarchVaRResult:
    ...
    try:
        am = arch_model(returns * 100, vol="Garch", p=1, q=1, dist="students-t", rescale=False)
        ...
        return _fallback_var(returns, confidence, details=f"GARCH failed: {e}")


def _fallback_var(returns: np.ndarray, confidence: float, details: str | None = None) -> GarchVaRResult:
    ...
    return GarchVaRResult(
        method="Static Normal (arch missing)",
        var_95=round(var_95, 4),
        var_99=round(var_99, 4),
